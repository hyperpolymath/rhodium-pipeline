= Security Policy
:author: Rhodium Pipeline Template Maintainers
:revdate: 2025-12-07
:toc: preamble
:toclevels: 2
:sectnums:
...
|1.x.x
|✓ Supported
|Current stable release; receives all security patches

|0.x.x
|✗ Pre-release
|Development versions; no security guarantees
|===
...
*Alternative:* Use GitLab's private vulnerability reporting feature on the project repository.
...
. We work with the reporter to understand and verify the issue.
. We develop and test a fix.
. We release the fix and publish a security advisory.
. After the fix is available, the vulnerability may be publicly disclosed.
...
* The project's https://gitlab.com/hyperpolymath/rhodium-pipeline-template/-/security/advisories[GitLab Security Advisories] page
* The `SECURITY-ADVISORIES.adoc` file in this repository
...
For template methodology questions: See link:docs/HANDOVER.adoc[HANDOVER.adoc]
